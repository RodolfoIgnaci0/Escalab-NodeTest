...
const {
  user: { storeUserSchema, updateUserSchema, userIDSchema, userLoginSchema, userRechargeBalance }
} = require('../../schemas')
...
UserRouter.route('/user/signup').post(
  ...
  async (req, res, next) => {
    try {
      const {
        body: { name, lastName, email, password, type }
      } = req
...
UserRouter.route('/user/:id/recharge')
.patch(
  validatorCompiler(userIDSchema, 'params'),
